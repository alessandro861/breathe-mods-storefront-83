...
const ModCard = ({ mod, isAdmin, onEdit, onDelete, onPurchase }: ModCardProps) => {
  ...
  return (
    <Card className="glass-panel overflow-hidden flex flex-col h-full">
      <div className="relative h-48 overflow-hidden">
        ...
        />
        {mod.isPaid && (
          <div className="absolute top-0 right-0 bg-primary/90 text-white px-2 py-1 text-xs font-semibold">
            {displayPrice()}
          </div>
        )}
      </div>
      ...
      <CardHeader className="pb-2">
        ...
        <CardDescription className="text-sm line-clamp-1">
          {mod.date && (
            <span className="flex items-center text-xs mb-1 text-muted-foreground">
      ...
      <CardFooter className="flex justify-between pt-2 border-t border-gray-700">
        ...
        <div className="flex gap-2">
          ...
          {mod.isPaid && (
            <Button size="sm" onClick={() => onPurchase(mod)}>
              Acheter
            </Button>
          )}
        </div>
